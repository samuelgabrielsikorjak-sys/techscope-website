...
document.addEventListener('DOMContentLoaded', function () {
  // nav: subtle shadow once the page scrolls under it
  var navEl = document.querySelector('.nav');
  if (navEl) {
    var onNavScroll = function () { navEl.classList.toggle('is-scrolled', window.scrollY > 8); };
    onNavScroll();
  ...
  if (toggle && links) {
    ...
    for (var n = 0; n < navAnchors.length; n++) {
      // the "Služby" dropdown trigger toggles its panel instead of closing
      // the whole menu — handled separately below.
      if (navAnchors[n].classList.contains('nav-dd-trigger')) continue;
      navAnchors[n].addEventListener('click', function () { links.classList.remove('open'); });
    }
  }

  // "Služby" rozbaľovacia karta — hover otvára na desktope (CSS), klik/klávesa
  // ju prepne (touch + klávesnica + mobil). Klik mimo ju zavrie.
  var navDd = document.querySelector('.nav-dd');
  if (navDd) {
    var ddTrigger = navDd.querySelector('.nav-dd-trigger');
    ddTrigger.addEventListener('click', function (e) {
      e.preventDefault();
      var open = navDd.classList.toggle('is-open');
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && navDd.classList.contains('is-open')) {
        navDd.classList.remove('is-open');
        ddTrigger.setAttribute('aria-expanded', 'false');
        ddTrigger.focus();
      }

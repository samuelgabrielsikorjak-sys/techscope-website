...
-- ============================================================================
-- crm_leads — jednoduchý CRM / pipeline nad leadmi
-- (admin.html → sekcia "CRM / Leady", assets/js/admin.js)
-- ============================================================================
-- Dva zdroje riadkov:
--   'rezervacny_formular' — vloží AUTOMATICKY trigger sync_lead_to_crm po
--     každom novom riadku v `leads` (t.j. po odoslaní rezervačného formulára,
--     ktorý ide cez RPC vytvorit_rezervaciu → insert do leads). povodny_lead_id
--     ukazuje späť na ten leads riadok kvôli needitovateľnému kontextu
--     (pôvodný dopyt, rozpočet, produkt, urgencia).
--   'manualny' — pridá admin ručne cez tlačidlo "+ Nový lead".
--
-- Celá sekcia je bezpečná spustiť opakovane (drop ... if exists pred create).

do $$ begin
  create type crm_lead_status as enum
    ('zaujemca', 'strategy_call', 'odoslana_ponuka', 'uzavrety', 'zruseny');
...
create table if not exists crm_leads (
  id uuid primary key default gen_random_uuid(),
  firma text null,
  kontaktna_osoba text null,
  email text null,
  telefon text null,
  status crm_lead_status not null default 'zaujemca',
  -- 'manualny' | 'rezervacny_formular'
  zdroj text not null default 'manualny',
  -- viaže riadok na pôvodný leads záznam; NULL pri manuálnych leadoch.
  -- unique => jeden crm_lead na jeden leads riadok (viac NULL je v Postgrese OK).
  povodny_lead_id uuid null unique references leads (id) on delete set null,
  poznamky text null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
drop policy if exists "Admin cita crm_leads" on crm_leads;
create policy "Admin cita crm_leads"
  on crm_leads for select to authenticated using (is_admin());

drop policy if exists "Admin vklada crm_leads" on crm_leads;
create policy "Admin vklada crm_leads"
  on crm_leads for insert to authenticated with check (is_admin());

drop policy if exists "Admin upravuje crm_leads" on crm_leads;
create policy "Admin upravuje crm_leads"
  on crm_leads for update to authenticated using (is_admin()) with check (is_admin());

drop policy if exists "Admin maze crm_leads" on crm_leads;
create policy "Admin maze crm_leads"
  on crm_leads for delete to authenticated using (is_admin());

-- ----------------------------------------------------------------------------
-- Trigger: nový leads riadok  ->  zrkadlový riadok v crm_leads
-- ----------------------------------------------------------------------------
-- SECURITY DEFINER => beží ako vlastník funkcie a obíde RLS na crm_leads,
-- takže insert prejde aj keď rezervačný formulár beží ako rola `anon`.
-- on conflict do nothing => idempotentné, keby trigger niekedy zbehol 2×.
...
begin
  insert into crm_leads (
    firma, kontaktna_osoba, email, telefon, status, zdroj, povodny_lead_id
  ) values (
    NEW.nazov_firmy, NEW.meno_priezvisko, NEW.email, NEW.telefon,
    'zaujemca', 'rezervacny_formular', NEW.id
  )
  on conflict (povodny_lead_id) do nothing;
  return NEW;
end;
$$;

drop trigger if exists trg_sync_lead_to_crm on leads;
create trigger trg_sync_lead_to_crm
  after insert on leads
  for each row execute function public.sync_lead_to_crm();

-- ---- SMOKE TEST triggera (spusti v Supabase SQL editore, celé naraz) --------
-- Vloží fiktívny lead, overí, že sa objavil v crm_leads, a upratá po sebe.
-- Musí vypísať jeden riadok so zdroj='rezervacny_formular' a status='zaujemca'.
-- do $$
-- declare v_id uuid;
-- begin
--   insert into leads (meno_priezvisko, nazov_firmy, pozicia, email, telefon,
--     rozpocet, urgencia, popis_projektu, gdpr_suhlas, status)
--   values ('SMOKE TEST', 'Smoke s.r.o.', 'CEO', 'smoke@test.local', '+421000',
--     '5-10k', 'stredna', 'test dopyt', true, 'novy')
--   returning id into v_id;
--   raise notice 'crm_leads riadok: %', (
--     select row(firma, kontaktna_osoba, zdroj, status, povodny_lead_id)
--     from crm_leads where povodny_lead_id = v_id);
--   delete from leads where id = v_id;  -- crm_leads.povodny_lead_id sa nastaví na NULL
--   delete from crm_leads where firma = 'Smoke s.r.o.';
-- end $$;

-- ============================================================================
-- FAKTÚRY (invoices) — generovanie z admin.html, sekcia "Prehľad zákaziek"
-- ============================================================================
-- Spustiť celý tento blok naraz v Supabase SQL editore.
--
-- Číslovanie: NIKDY nezávisí od počtu riadkov v `invoices` (to by sa pokazilo
-- po zmazaní testovacej faktúry). Beží cez samostatné počítadlo
-- `invoice_counters` (1 riadok na rok) a atomickú funkciu
-- generuj_cislo_faktury(). Zmazanie faktúry počítadlo NEznižuje — rad čísel
-- je súvislý a číslo sa nikdy nezopakuje (medzery po storne sú OK, § 71 z. 222/2004).

-- ---- počítadlo (jeden riadok na kalendárny rok) ----------------------------
create table if not exists invoice_counters (
  rok            int  primary key,
  posledne_cislo int  not null default 0
);
alter table invoice_counters enable row level security;
-- žiadne RLS politiky: cez klientský SDK je tabuľka neprístupná, mení ju len
-- funkcia nižšie (security definer) a service_role v dashboarde.

-- ---- atomický generátor čísla ---------------------------------------------
-- INSERT ... ON CONFLICT DO UPDATE ... RETURNING je JEDEN príkaz: Postgres
-- pri konflikte zamkne existujúci riadok počítadla, takže dve súbežné
-- transakcie sa serializujú — druhá počká na commit prvej, znova načíta
-- posledne_cislo a inkrementuje. Duplicitné číslo tak nemôže vzniknúť ani
-- pri dvoch faktúrach vystavených v tej istej milisekunde.
create or replace function public.generuj_cislo_faktury()
returns text
language plpgsql
security definer
set search_path = public
as $$
declare
  v_rok   int := extract(year from now())::int;
  v_cislo int;
begin
  if not public.is_admin() then
    raise exception 'Len admin môže generovať číslo faktúry';
  end if;

  insert into invoice_counters (rok, posledne_cislo)
  values (v_rok, 1)
  on conflict (rok)
  do update set posledne_cislo = invoice_counters.posledne_cislo + 1
  returning posledne_cislo into v_cislo;

  return v_rok::text || '/' || lpad(v_cislo::text, 3, '0');  -- napr. 2026/001
end;
$$;
grant execute on function public.generuj_cislo_faktury() to authenticated;

-- ---- tabuľka faktúr -------------------------------------------------------
create table if not exists invoices (
  id            uuid primary key default gen_random_uuid(),
  project_id    uuid not null references projects (id) on delete cascade,
  cislo_faktury text not null unique,      -- vždy z generuj_cislo_faktury(), NIKDY ručne

  -- dodávateľ (zatiaľ placeholder — bez registrovaného IČO/DIČ)
  dodavatel_meno    text,
  ...
  -- odberateľ (natiahnutý z projektu/klienta, editovateľný pri generovaní)
  odberatel_meno    text,
  odberatel_adresa  text,
  odberatel_ico     text,
  odberatel_dic     text,

  polozky      jsonb   not null default '[]'::jsonb,  -- [{popis, mnozstvo, jednotkova_cena}]

  je_platca_dph boolean       not null default false,  -- prepínač "Nie som platcom DPH" => false
  zaklad_dane   numeric(12,2) not null default 0,
  sadzba_dph    numeric(5,2)  not null default 0,
  vyska_dph     numeric(12,2) not null default 0,
  celkova_suma  numeric(12,2) not null default 0,

  datum_vystavenia date not null default current_date,
  datum_dodania    date,
  ...
  status     text not null default 'vystavena'
             check (status in ('vystavena', 'zaplatena', 'po_splatnosti')),
  pdf_path   text,                                   -- cesta v bucket-e 'invoices'
  created_at timestamptz not null default now()
);
...
create policy "Admin cita faktury"
  on invoices for select to authenticated using (is_admin());
create policy "Admin vklada faktury"
...
create policy "Admin maze faktury"
  on invoices for delete to authenticated using (is_admin());

-- ---- triggery: číslo sa VŽDY generuje funkciou a je nemenné ---------------
create or replace function public.invoices_set_cislo()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  new.cislo_faktury := public.generuj_cislo_faktury();  -- ignoruje čokoľvek poslané z klienta
  return new;
end;
$$;
drop trigger if exists trg_invoices_set_cislo on invoices;
create trigger trg_invoices_set_cislo
  before insert on invoices
  for each row execute function public.invoices_set_cislo();

create or replace function public.invoices_lock_cislo()
returns trigger language plpgsql as $$
begin
  new.cislo_faktury := old.cislo_faktury;  -- číslo sa po vytvorení nedá zmeniť
  return new;
end;
$$;
drop trigger if exists trg_invoices_lock_cislo on invoices;
create trigger trg_invoices_lock_cislo
  before update on invoices
  for each row execute function public.invoices_lock_cislo();

-- ---- Storage bucket 'invoices' (privátny) + politiky ---------------------
insert into storage.buckets (id, name, public)
values ('invoices', 'invoices', false)
on conflict (id) do nothing;

create policy "Admin cita faktury v buckete"
  on storage.objects for select to authenticated
  using (bucket_id = 'invoices' and is_admin());
create policy "Admin spravuje faktury v buckete"
  on storage.objects for all to authenticated
  using (bucket_id = 'invoices' and is_admin())
  with check (bucket_id = 'invoices' and is_admin());

-- ---- SMOKE TEST súvislého číslovania (spusti celé naraz) ------------------
-- Musí vypísať tri po sebe idúce čísla (napr. 2026/001, 2026/002, 2026/003)
-- a po zmazaní prostrednej faktúry ďalšie číslo 2026/004 (nie 002 znova).
-- do $$
-- declare a text; b text; c text; d text; pid uuid;
-- begin
--   select id into pid from projects limit 1;
--   insert into invoices (project_id, polozky) values (pid, '[]') returning cislo_faktury into a;
--   insert into invoices (project_id, polozky) values (pid, '[]') returning cislo_faktury into b;
--   insert into invoices (project_id, polozky) values (pid, '[]') returning cislo_faktury into c;
--   delete from invoices where cislo_faktury = b;                 -- zmaž prostrednú
--   insert into invoices (project_id, polozky) values (pid, '[]') returning cislo_faktury into d;
